...
```yaml
- uses: EmadMokhtar/skill-evaluator@v0.8.0
  with:
    path: ./skills
    runner: pydantic-ai
...
| --- | --- | --- |
| `install-spec` | `skill-lens[pydantic-ai]==0.8.0` | Passed verbatim to `uv tool install`. Accepts a PyPI name, a pinned version, a git ref, or a local path. Add the `langchain` extra (`skill-lens[pydantic-ai,langchain]`, pinned the same way) when a job runs the LangChain runner. |
...
```yaml
      - uses: EmadMokhtar/skill-evaluator@v0.8.0
        with:
          path: ./skills
          install-spec: "skill-lens[pydantic-ai,langchain]==0.8.0"
